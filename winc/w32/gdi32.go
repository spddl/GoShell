...
var (
	...
	enumFontFamiliesEx            = modgdi32.NewProc("EnumFontFamiliesExW")
)
...
func SetBkColor(hdc HDC, crColor COLORREF) COLORREF {
	ret, _, e := procSetBkColor.Call(
		uintptr(hdc),
		uintptr(crColor))

	if ret == CLR_INVALID {
		panic("SetBkColor failed: " + e.Error())
	}
...
func CreateCompatibleBitmap(hdc HDC, nWidth, nHeight int32) HBITMAP {
	ret, _, _ := syscall.Syscall(procCreateCompatibleBitmap.Addr(), 3,
		uintptr(hdc),
		uintptr(nWidth),
		uintptr(nHeight))
...
func GdiFlush() bool {
	ret, _, _ := syscall.Syscall(procGdiFlush.Addr(), 0,
		0,
		0,
		0)
...
func SetViewportOrgEx(hdc HDC, x, y int32, lpPoint *POINT) COLORREF {
	ret, _, _ := syscall.SyscallN(procSetViewportOrgEx.Addr(),
		uintptr(hdc),
		uintptr(x),
		uintptr(y),
		uintptr(unsafe.Pointer(lpPoint)))
