...
func (ca *Canvas) GetHwnd() w32.HWND {
	return ca.hdc
}
...
func (ca *Canvas) SetStretchBltMode(iStretchMode int) int {
	return w32.SetStretchBltMode(ca.hdc, iStretchMode)
}
...
func (ca *Canvas) DrawIconEx(ico *Icon, x, y int32, cxWidth int32, cyWidth int32, istepIfAniCur uint32, hbrFlickerFreeDraw uintptr, diFlags uint32) bool {
	return w32.DrawIconEx(ca.hdc, x, y, ico.Handle(), cxWidth, cyWidth, istepIfAniCur, hbrFlickerFreeDraw, diFlags)
}
...
// https://learn.microsoft.com/en-us/previous-versions/ms997507(v=msdn.10)
func (ca *Canvas) SetTransparency(transperancy byte) {
	wAttr := w32.GetWindowLong(ca.hwnd, w32.GWL_EXSTYLE)
	w32.SetWindowLong(ca.hwnd, w32.GWL_EXSTYLE, uint32(wAttr|w32.WS_EX_LAYERED))
	w32.SetLayeredWindowAttributes(ca.hwnd, 0, transperancy, 0x02)
}

// https://learn.microsoft.com/en-us/windows/win32/api/wingdi/nf-wingdi-setbkmode
func (ca *Canvas) SetBkColor(mode uint32) uint32 {
	return w32.SetBkColor(ca.hdc, mode)
}
