...
// 	f, _ := syscall.UTF16FromString(fnt.family)
// 	src := f
// 	dest := lf.FaceName[:]
// 	copy(dest, src)
...
func CreateDPIAwareFont(fontName string, points int) *GdiFont {
	gdiFont := &GdiFont{}
	var lfFont w32.LOGFONT
	src, _ := syscall.UTF16FromString(fontName)
	dest := lfFont.FaceName[:]
	copy(dest, src)

	var FontReferenceDPI int = 72
	var DPI int = 96
	lfFont.Height = int32(-w32.MulDiv(points, DPI, FontReferenceDPI))
	lfFont.Weight = w32.FW_LIGHT
	lfFont.CharSet = w32.ANSI_CHARSET
	lfFont.OutPrecision = w32.OUT_DEFAULT_PRECIS
	lfFont.ClipPrecision = w32.CLIP_DEFAULT_PRECIS
	lfFont.Quality = w32.CLEARTYPE_QUALITY
	gdiFont.hfont = w32.CreateFontIndirect(&lfFont)
	return gdiFont
}

func (f *GdiFont) GetHandle() w32.HFONT {
	return f.hfont
}
